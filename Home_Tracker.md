...
**Shipped in v2.3.40 (2026-05-12 — tag pushed, CurseForge + Wago published):**
...
- HS-081 #38 map-pipeline taint — origin A (map-pin tooltip) `3d55565` + origin B (MapSidePanel `WorldMapFrame` hooks → passive `C_Timer` poll) `dec9c04`; Gate 2 PASS 2026-05-12; #38 closed — **moved to `Home_Completed.md`**
- HS-014 ProfessionSources `skillTier` backfill `68885dc` + HS-080 misclassified-shop-item correction (HS-080 **moved to `Home_Completed.md`**) + item-246604 `recipeName` trailing-space fix (`Data/ProfessionSources.lua` regen)
...
- ▶ **HS-079 — profession sub-phase: Gate 0 + spike done (2026-05-12); ready to write the `PLAN_TEMPLATE.md` plan.**
  ...
  skillTier merge), current; the `(profession, expansionTier) → uiMapID + coords`
  trainer-location table has no capture path and must be hand-curated (Wowhead + `/way`; originality
  rule applies). Spike output in hand — `Home_Dev/scripts/exports/profession_recipes.tsv` (308 `R` rows /
  1285 `G` rows, committed Home_Dev `8cbee61`), reviewed 2026-05-12: **no decor recipe needs an exotic
  crafting station** (all common spell focuses / inventory tools) → the location table reduces to trainer
  locations only (answers HS-075 Q1); reagent rows feed HS-076. **Next:** write the profession-sub-phase
  `PLAN_TEMPLATE.md` plan with that data in hand (Tier 2). Drops/quests/achievements sub-phases stay deferred.
...
- **Status:** Backlog — **profession sub-phase Gate 0 + spike done (2026-05-12); ready to write the `PLAN_TEMPLATE.md` plan.** HS-014 done → profession baseline unblocked. Gate 0 confirmed the HS-018 plug-in contract (collector `pinSourceProviders.profession.collect(self, mapID, validMapIDs, filter, renderState)` pushing records into `renderState.sourcePins`; a `"profession"` branch in `PinFrameFactory:CreateSourcePinFrame`; renderer integration free) and reframed the data question: **no in-game scanner exists for profession/recipe/trainer data** — `ProfessionSources` is pipeline-built (Web API decor `source[].recipes` + `C_HousingCatalog` sourceText parse + the HS-014 skillTier merge), current; the `(profession, expansionTier) → uiMapID + coords` trainer-location table has no capture path and must be hand-curated (Wowhead + manual `/way`; originality rule — don't import HDG's data). Spike done: `/hsdev exportprofessionrecipes` (Homestead-Dev `f025cd7`) dumped per-recipe `GetRecipeInfo` + `GetRecipeRequirements` + `GetRecipeSchematic` → `Home_Dev/scripts/exports/profession_recipes.tsv` (308 `R` rows / 1285 `G` rows; committed Home_Dev `8cbee61`; stray dup at `Home_Dev/scripts/profession_recipes.tsv` is intentional). Reviewed 2026-05-12: **no decor recipe needs an exotic crafting station** — every `requirements` row is a common spell focus or an inventory tool, so the location table reduces to trainer locations only (answers HS-075 Q1); the reagent (`G`) rows feed HS-076's reverse index. (One byproduct: the spike surfaced the item-246604 trailing-space `recipeName` bug → fixed in the 2026-05-12 release.) **Next action:** write the profession-sub-phase `PLAN_TEMPLATE.md` plan with this data in hand. Drops/quests/achievements sub-phases stay deferred behind the profession slot. Tier 2.
