...
- **Status:** Origin A — **Done**, merged `3d55565`, in-game confirmed, Argus PASS, pushed. Origin B — **Backlog**, ready for a focused worktree (bisect patch + findings doc already prepared, see Notes).
...
- **Notes / artifacts:** the `Homestead-issue38-taint` worktree (`C:\Projects\Homestead-issue38-taint`, branch `investigate/issue38-map-taint`, off `main`) holds `ISSUE38-TAINT-FINDINGS.md` (full write-up) and `bisect-patches/` — in particular `bisect-patches/C-disable-mapsidepanel-worldmapframe-hooks.patch` (a `BISECT38_NO_WORLDMAP_HOOKS` flag that disables all six hooks) to confirm origin B in-game before/while doing the rewrite. Clean that worktree up once origin B lands or is dropped: `git -C C:/Projects/Homestead worktree remove Homestead-issue38-taint && git branch -D investigate/issue38-map-taint`. Closest other item: HS-036 (VendorMapPins runtime lifecycle QA) — predates HS-018, different scope.
