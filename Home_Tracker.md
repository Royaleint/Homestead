...
- **Type:** Feature
- **Priority:** Medium
- **Status:** Backlog — plan Revision 2 ready for Douglock (`Home_Dev/plans/active/HS-018-source-aware-map-filtering.md` @ Home_Dev `fac7f9d`). Gate 3 closed 2026-05-10; Rawb code review surfaced 7 gaps addressed in Revision 2. Next action: Douglock creates worktree `feature/source-aware-map-filtering`, implements 3 commits in order.
...
- **Session context (2026-05-10):** Tier 2, 3 commits planned (~110 LOC total) on branch `feature/source-aware-map-filtering`. Existing plumbing already 60% there — `BadgeCalculation.GetZoneVendorCounts(continentMapID, sourceFilter)` accepts and caches on filter; 7 call sites in `VendorMapPins.lua` (L518, L522, L645, L1380, L1403, L1430, L1453, L1479, L1508) and `PinFrameFactory.lua:217` drop the argument. Plan introduces a `pinSourceProviders` registry on `VendorMapPins` as the HS-075 inheritance contract.
- **Gate 3 resolutions (2026-05-10):** Q1 drops do NOT contribute to badges (defer until full location data) → BadgeCalculation untouched. Q2 sequential HS-018 → HS-022. Q3 HS-075 owns profession trainer data + filtering logic. Q4 minimap untouched in HS-018 (`HomesteadMinimapOverlay` not modified). Q5 sprint order confirmed.
