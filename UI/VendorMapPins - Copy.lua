...
local zoneToContinent = {
    ...
    -- Brawler's Guild (special instances)
...
local function HasValidCoordinates(vendor)
    if not vendor.coords or not vendor.coords.x or not vendor.coords.y then
        return false
    end
    -- Skip placeholder coordinates (exactly 0.5, 0.5)
    if vendor.coords.x == 0.5 and vendor.coords.y == 0.5 then
        return false
    end
    -- Also skip coordinates of exactly 0.50 (string comparison edge case)
    if vendor.coords.x == 0.50 and vendor.coords.y == 0.50 then
        return false
    end
    return true
end
...
function VendorMapPins:GetZoneVendorCounts(continentMapID)
    ...
    for _, vendor in ipairs(allVendors) do
        local zoneMapID = vendor.mapID
        local continent = GetContinentForZone(zoneMapID)

        -- Only count vendors with valid coordinates
        if continent == continentMapID and HasValidCoordinates(vendor) then
...
function VendorMapPins:GetContinentVendorCounts()
    ...
    for _, vendor in ipairs(allVendors) do
        ...
        -- Only count vendors with valid coordinates
        if continentMapID and HasValidCoordinates(vendor) then
            local canAccess = self:CanAccessVendor(vendor)
            local isOpposite = self:IsOppositeFaction(vendor)
    ...
    for _, mapID in ipairs(mapsToCheck) do
        ...
        if vendors then
            for _, vendor in ipairs(vendors) do
                ...
                if HasValidCoordinates(vendor) and vendor.npcID and not addedVendors[vendor.npcID] then
                    local canAccess = self:CanAccessVendor(vendor)
                    local isOpposite = self:IsOppositeFaction(vendor)
                    ...
                    if canAccess or (isOpposite and showOpposite) then
                        ...
                        -- Use vendor's actual mapID for correct distance calculation
                        -- HereBeDragons will only show the pin if it's within minimap range
                        local vendorMapID = vendor.mapID or mapID
                        ...
                        HBDPins:AddMinimapIconMap("HomesteadMinimapVendors", frame, vendorMapID,
                            vendor.coords.x, vendor.coords.y,
...
function VendorMapPins:ShowVendorPins(mapID)
    ...
    for _, vendor in ipairs(vendors) do
        -- Only show pins for vendors with valid (non-placeholder) coordinates
        if HasValidCoordinates(vendor) then
            local canAccess = self:CanAccessVendor(vendor)
            local isOpposite = self:IsOppositeFaction(vendor)
